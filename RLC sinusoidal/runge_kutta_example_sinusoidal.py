...
if __name__ == "__main__":
    ...
    m = np.array([10], dtype='float32')
    c = np.array([30], dtype='float32') # initial guess
    k = np.array([2.85], dtype='float32')

    # data
    df = pd.read_csv('./data/data_sinusoide2T.csv')
    t  = df[['t']].values
    dt = (t[1] - t[0])[0]
    ...
    initial_state = np.array([[ -7.4363e-04,-0.1486]] ,dtype='float32' )
    ...
    history = model.fit(utrain, ytrain, epochs=5000, steps_per_epoch=1, verbose=1, callbacks=[mckp])
